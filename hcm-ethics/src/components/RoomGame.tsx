...
export default function RoomGame({ snapshot, credential, accept, refresh, connectionError }: {
  ...
  return <div className="play-layout">
    {/* Cột trái: Lịch sử điểm của bạn */}
    <aside className="play-col-left">
      <ScoreHistory key={me.id} history={me.history} credential={credential} />
      <details className="rules panel">
        <summary>Luật chơi & cách tính điểm</summary>
        <p>Mỗi người chỉ chơi một ván mỗi phiên. Mỗi 3 lượt của bạn mở một câu quiz. Đúng +30 điểm và chọn thẻ; sai mất 50% điểm.</p>
        <p>Thắng +100, hòa +50, thua +20. Thắng dưới 15 nước cả hai bên được thêm 50 điểm. Hết ván không chơi lại; bảng điểm vẫn cập nhật thẻ tác động từ người khác đến hết phiên.</p>
        <p>Thẻ có thể cộng, trừ, nhân đôi, cướp, chia điểm hoặc đóng băng. Điểm tối đa một tỷ. Cùng điểm: người vào phòng trước xếp trên.</p>
      </details>
    </aside>

    {/* Cột giữa: Bàn cờ Caro & Thông tin người chơi */}
    <section className="board-panel panel">
      <div className="board-topline">
        <div>
          <p className="eyebrow">LƯỢT CHƠI DUY NHẤT · BẠN LÀ <span className="text-x">X</span></p>
          <h1>{busy ? pending.type === "move" ? "Bot đang đi…" : "Đang lưu thao tác…" : game.stage === "move" ? "Đến lượt bạn." : game.stage === "round" ? game.result === "win" ? "Bạn thắng ván này!" : game.result === "lose" ? "Bot thắng ván này." : "Ván cờ hòa." : game.stage === "frozen" ? "Đóng băng 8 giây" : "Chinh phục thử thách"}</h1>
        </div>
      ...
      <div className="board-progress">
        <span>{game.stage === "round" ? "Ván đã hoàn thành" : game.stage === "move" ? "Mỗi 3 lượt mở một quiz" : "Thử thách của bạn"}</span>
        <div className="quiz-pips" aria-hidden="true">{[0, 1, 2].map((pip) => <i key={pip} className={game.stage !== "move" || Math.ceil(game.roundMoves / 2) % 3 > pip ? "filled" : ""} />)}</div>
      </div>
      <div className="board-stage">
        <GameBoard board={board} winnerState={game.winner} disabled={busy || expired || Boolean(connectionError) || game.stage !== "move"} onCellClick={(row, col) => { void act({ type: "move", row, col }); }} />
        {(game.stage === "frozen" || expired) && <div className="board-cover">
          <strong>{expired ? "Đã hết thời gian" : "Chờ tan băng"}</strong>
          {game.stage === "frozen" && !expired && <RoomClock endsAt={new Date(game.frozenUntil).toISOString()} serverTime={snapshot.serverTime} />}
          {(error || connectionError) && <button className="button" onClick={() => { void act({ type: "thaw" }); }}>Thử lại</button>}
        </div>}
      </div>
      <div className="board-caption">
        <span><i className="legend-x" /> Bạn · X</span>
        <span><i className="legend-o" /> Bot · O</span>
        <span>Nối 5 quân để thắng</span>
      </div>
    ...
    {/* Cột phải: Bảng xếp hạng & Đồng hồ phiên */}
    <aside className="play-col-right">
      <div className="session-timer panel">
        <div>
          <p className="eyebrow">THỜI GIAN CÒN LẠI</p>
          <span>Phòng {snapshot.room.code}</span>
        </div>
        <RoomClock endsAt={snapshot.room.ends_at} serverTime={snapshot.serverTime} onExpire={() => { setExpired(true); refresh(); }} />
      </div>
    ...
    <QuizModal game={game} pending={pending} error={error || connectionError} onAnswer={(index) => { void act({ type: "answer", index }); }} onContinue={() => { void act({ type: "continue" }); }} onCard={(index) => { void act({ type: "card", index }); }} onAcknowledge={() => { void act({ type: "acknowledge" }); }} />
  </div>;
}
